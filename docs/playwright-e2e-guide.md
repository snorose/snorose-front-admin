...
실제 브라우저 창에서 조회 테스트의 동작을 보려면 다음 명령을 사용합니다.

```bash
npm run test:e2e:headed
```

이 명령은 `dev-read`와 의존하는 로그인 setup을 실행합니다. 특정 기능의 파일 경로를 `--` 뒤에 붙여 실행 범위를 줄일 수 있습니다. 자세한 예시는 [E2E README](../e2e/README.md#브라우저를-보면서-실행하기)를 참고합니다.

각 테스트 단계를 확인하려면 UI 모드를 사용합니다.
