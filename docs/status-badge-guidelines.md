...
현재 1~5단계의 상태 배지 공통화 작업이 모두 완료되었다.
...
| 공지 배지             | 정보성 상태        | `info` tone 검토                                   |
| 의심 키워드 `Y`·`N`   | 검토 상태          | `있음`은 `warning`, `없음`은 `neutral`로 교체 검토 |
| 신고 건수 배지        | 위험도 표시        | `danger` tone 검토                                 |
| 징계 종류 배지        | 제재 상태          | `danger` tone 사용                                 |
