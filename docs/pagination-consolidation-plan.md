...
- 회원 목록의 전체 페이지 수가 줄어든 경우 마지막 유효 페이지로 이동하는 범위 보정과 요청 경합 방지 테스트를 추가했다.
...
### 5.5 응답 이후 현재 페이지 범위·요청 경합 보정 — 완료
...
페이지 또는 필터 요청이 겹칠 때는 요청 ID를 기준으로 최신 요청의 응답만 `members`, 페이지 정보, 선택 상태와 로딩 상태에 반영한다. 이전 요청이 최신 요청보다 먼저 또는 나중에 완료되는 두 흐름을 단위 테스트로 검증했다.
...
- [`src/domains/MemberInfo/hooks/useMemberDirectoryState.ts`](../src/domains/MemberInfo/hooks/useMemberDirectoryState.ts)
  ...
  - **완료:** 요청이 겹치면 최신 요청만 목록·페이지·로딩 상태를 갱신한다.
...
8. **완료:** 페이지·필터 요청이 겹쳐도 이전 응답이 최신 목록과 페이지 상태를 덮어쓰지 않도록 한다.
...
- [x] 겹친 요청의 응답 순서가 바뀌어도 최신 회원 목록·페이지·로딩 상태를 유지한다.
...
- [x] 전체 테스트 147개를 실행한다. (2026-08-23 통과)
