...
### 5.3 페이지 묶음 크기와 데이터 개수를 분리한다 — 결정 및 추가 작업 필요
...
- [`src/shared/components/PaginationBar.tsx`](../src/shared/components/PaginationBar.tsx)
  ...
  - **추가 작업 필요:** 회원 이력의 5개 묶음을 유지하기로 하면 `pageBlockSize`를 추가한다.
...
- `pageBlockSize`를 도입하면 5개 묶음과 10개 묶음을 각각 검증한다. 이 항목은 prop 도입을 선택할 때 추가한다.
...
| 5개 페이지 묶음 지원   | **결정 필요**        | `/member/penalty`의 기존 5개 묶음을 유지할지, 공통 10개 묶음으로 통일할지 결정해야 한다.                         |
| `pageBlockSize` prop   | **조건부 추가 작업** | 5개 묶음을 유지한다면 선택 prop과 테스트가 필요하다. 10개로 통일하면 생략한다.                                   |
| 빈 결과·한 페이지 노출 | **현행 유지로 결정** | 공통 컴포넌트는 1번 버튼을 표시하고, 숨김이 필요한 화면은 호출부가 결정한다. 동작은 단위 테스트로 고정했다.      |
| 비활성 링크 접근성     | **선택 개선**        | CSS 비활성화에 더해 `aria-disabled`, `tabIndex`, 키보드 활성화 차단을 적용할 수 있다. 통합 완료 조건과는 별도다. |
| 이전·다음 버튼 언어    | **선택 개선**        | 현재 `Previous`, `Next` 영문 접근성 문구를 한글화할지는 프로젝트 전역 언어 정책과 함께 결정한다.                 |
...
- [ ] **결정 필요:** 5개 페이지 묶음을 유지해야 하는지 결정한다.
- [ ] **조건부 추가 작업:** 필요하면 `pageBlockSize`를 추가하고 기본값은 10으로 둔다.
- [ ] **선택 개선:** 비활성 링크에 `aria-disabled`와 키보드 접근 정책을 적용한다.
- [ ] **선택 개선:** 이전/다음 버튼의 영문 레이블과 접근성 문구를 프로젝트 언어 정책에 맞춘다.
...
- [ ] **결정 필요:** 페이지 번호 묶음도 5개로 유지할지 결정한다.
...
- [x] 관련 단위 테스트 23개를 실행한다. (2026-08-23 통과)
